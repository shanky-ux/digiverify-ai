...
# Optional BERT-based reasoning — disabled by default (set ENABLE_BERT=1 to enable)
bert_reasoner = None
if os.environ.get('ENABLE_BERT', '0') == '1':
    try:
...
app.config['SQLALCHEMY_DATABASE_URI'] = os.environ.get(
    'DATABASE_URL',
    'mysql+pymysql://root:root@localhost:3306/welfare_system'
)
...
def _record_on_chain(event_type, user_id, data=None, recorded_by='system'):
    """Record an event on the DigiVerify blockchain (non-blocking on failure).
    Returns (block_hash, block_index) or (None, None) if service is unavailable."""
    try:
        payload = _json.dumps({
            'type':       event_type,
            'userId':     int(user_id),
        ...
        print(f'[Blockchain] Warning: could not record {event_type} — {exc}')
        return None, None

def allowed_file(filename, allowed_exts):
    return '.' in filename and filename.rsplit('.', 1)[1].lower() in allowed_exts

def save_upload(file_obj, sub_folder, allowed_exts):
    """Save an uploaded file and return the relative path or raise ValueError."""
    ...
    target_dir = os.path.join(app.config['UPLOAD_FOLDER'], sub_folder)
    os.makedirs(target_dir, exist_ok=True)
    ext = file_obj.filename.rsplit('.', 1)[1].lower()
    filename = f'{uuid.uuid4().hex}.{ext}'
    file_obj.save(os.path.join(target_dir, filename))
    return f'uploads/{sub_folder}/{filename}'
...
class BeneficiaryVerification(db.Model):
    """
    Stores the full verification record for a beneficiary.
    - If the person is alive: aadhaar_doc, pan_doc, selfie are required + a live video call.
    - If the person is deceased: death_cert_path is required.
    """
    __tablename__ = 'beneficiary_verifications'
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=False)
    beneficiary_type = db.Column(db.String(10), default='alive')   # 'alive' | 'deceased'
    # Document paths
    aadhaar_doc_path   = db.Column(db.Text)   # uploaded Aadhaar card image
    pan_doc_path       = db.Column(db.Text)   # uploaded PAN card image
    selfie_path        = db.Column(db.Text)   # live selfie photo
    death_cert_path    = db.Column(db.Text)   # death certificate (for deceased)
    # PAN number entered by user
    pan_number         = db.Column(db.String(20))
    # Face-match score (0-100) computed server-side
    match_score        = db.Column(db.Float)
    # Jitsi video call
    video_room_id      = db.Column(db.String(100))
    video_room_url     = db.Column(db.Text)
    video_status       = db.Column(db.String(20), default='not_scheduled')   # not_scheduled | scheduled | completed | failed
    # Overall verification status
    status             = db.Column(db.String(20), default='pending')  # pending | approved | rejected | pending_video
    admin_remarks      = db.Column(db.Text)
    # Blockchain certificate — set after the event is mined on-chain
...
# ── Submit verification (alive beneficiary) ────────────────────────────────────
@app.route('/verify/submit', methods=['POST'])
def verify_submit():
    """
    Multipart form upload for alive beneficiaries.
    Form fields: user_id, pan_number
    Files: aadhaar_doc, pan_doc, selfie
    """
    ...
    # Validate PAN format (basic: 10 alphanumeric, e.g. ABCDE1234F)
    import re
    if pan_num and not re.fullmatch(r'[A-Z]{5}[0-9]{4}[A-Z]', pan_num):
        return jsonify({'error': 'Invalid PAN number format (expected e.g. ABCDE1234F)'}), 400
    ...
    # Compute a simple match score (placeholder — replace with real face comparison)
    match_score = _compute_match_score(selfie_path)

    # Determine initial status
    if match_score >= 70:
        status = 'pending_video'          # good photo match → schedule video call
    elif match_score >= 40:
        status = 'pending_video'          # moderate → still require video
    else:
        status = 'rejected'               # too low — reject outright

    # Generate Jitsi room
    room_id  = f'digiverify-{uuid.uuid4().hex[:12]}'
    room_url = f'https://meet.jit.si/{room_id}'

    verif = BeneficiaryVerification(
        user_id          = int(user_id),
        beneficiary_type = 'alive',
        aadhaar_doc_path = aadhaar_path,
    ...
    _notify(int(user_id),
            ...
            f'{"Please complete the live video call with the scheme admin." if status == "pending_video" else "Unfortunately your documents were rejected — please resubmit."}',
            'verification')
    db.session.commit()

    # Record document submission on the blockchain
    ...
    if block_hash:
        verif.block_hash  = block_hash
        verif.block_index = block_index
        db.session.commit()
...
# ── Submit death certificate (deceased beneficiary) ────────────────────────────
@app.route('/verify/death-certificate', methods=['POST'])
def verify_death_cert():
    """
    Multipart upload for deceased beneficiaries.
    Form fields: user_id
    File: death_cert
    """
    user_id = request.form.get('user_id')
    ...
    # Check if a pending record already exists; update it
    verif = BeneficiaryVerification.query.filter_by(
        user_id=int(user_id), beneficiary_type='deceased'
    ).filter(BeneficiaryVerification.status.in_(['pending', 'pending_video'])).first()

    if not verif:
        verif = BeneficiaryVerification(
            user_id          = int(user_id),
            beneficiary_type = 'deceased',
            death_cert_path  = cert_path,
            status           = 'pending',
            created_at       = datetime.utcnow(),
            updated_at       = datetime.utcnow(),
        )
        ...
        verif.death_cert_path = cert_path
        verif.updated_at      = datetime.utcnow()

    _notify(int(user_id),
            'Death certificate uploaded successfully. An admin will review and verify it shortly.',
            'verification')
    db.session.commit()
    ...
    return jsonify({
        'id':      verif.id,
        'status':  verif.status,
        'message': 'Death certificate uploaded. Pending admin review.',
    }), 201


# ── User: request / get video call link ───────────────────────────────────────
@app.route('/verify/video-link/<int:user_id>', methods=['GET'])
def verify_video_link(user_id):
    """Return the latest video room URL for a user so they can join."""
    verif = (BeneficiaryVerification.query
             .filter_by(user_id=user_id)
             .order_by(BeneficiaryVerification.created_at.desc())
             .first())
    if not verif:
        return jsonify({'error': 'No verification found for this user'}), 404
    return jsonify({
        'id':            verif.id,
        'status':        verif.status,
        'video_room_url': verif.video_room_url,
        'video_status':  verif.video_status,
        'match_score':   verif.match_score,
    })
...
def verify_status(user_id):
    ...
    return jsonify(_verif_to_dict(verif))


# ── Admin: list all verifications ─────────────────────────────────────────────
@app.route('/admin/verifications', methods=['GET'])
def admin_all_verifications():
    verifs = BeneficiaryVerification.query.order_by(
        BeneficiaryVerification.created_at.desc()).all()
    return jsonify([_verif_to_dict(v) for v in verifs])


# ── Admin: pending video calls ─────────────────────────────────────────────────
@app.route('/admin/verifications/pending-video', methods=['GET'])
def admin_pending_video():
    verifs = BeneficiaryVerification.query.filter_by(status='pending_video').order_by(
        BeneficiaryVerification.created_at.asc()).all()
    return jsonify([_verif_to_dict(v) for v in verifs])
...
def admin_approve_verification(verif_id):
    verif = BeneficiaryVerification.query.get(verif_id)
    if not verif:
        return jsonify({'error': 'Not found'}), 404
    data = request.json or {}
    verif.status       = 'approved'
    verif.admin_remarks = data.get('remarks', 'Verification approved by admin')
    verif.updated_at   = datetime.utcnow()

    # Mark Aadhaar as verified in user_identity
    identity = UserIdentity.query.filter_by(user_id=verif.user_id).first()
    if identity:
        identity.is_aadhaar_verified = True
    else:
        db.session.add(UserIdentity(user_id=verif.user_id, is_aadhaar_verified=True,
                                    pan_number=verif.pan_number))

    _notify(verif.user_id,
            'Your identity verification has been APPROVED. You can now access welfare schemes.',
            'verification')
    db.session.commit()

    # Record approval on the blockchain
    block_hash, block_index = _record_on_chain(
        'VERIFICATION_APPROVED', verif.user_id,
        {
            'pan_number':    verif.pan_number,
            'admin_remarks': verif.admin_remarks,
            'match_score':   verif.match_score,
        },
        recorded_by='admin'
    )
    ...
    return jsonify({'id': verif_id, 'status': 'approved',
...
def admin_reject_verification(verif_id):
    ...
    verif.admin_remarks = data.get('remarks', 'Verification rejected by admin')
    verif.updated_at   = datetime.utcnow()
    _notify(verif.user_id,
            f'Your verification was rejected. Reason: {verif.admin_remarks}. Please resubmit.',
            'verification')
    db.session.commit()

    # Record rejection on the blockchain
    _record_on_chain(
        'VERIFICATION_REJECTED', verif.user_id,
        {'admin_remarks': verif.admin_remarks},
        recorded_by='admin'
    )
...
# ── Admin: record video call result ───────────────────────────────────────────
...
def admin_video_result(verif_id):
    ...
    verif.video_status  = 'completed'
    verif.status        = 'approved' if passed else 'rejected'
    verif.admin_remarks = data.get('remarks', '')
    verif.updated_at    = datetime.utcnow()
    ...
    msg = ('Your live video verification PASSED. Your account is now fully verified!'
           if passed else
           'Your live video verification FAILED. Please contact support or resubmit documents.')
    _notify(verif.user_id, msg, 'verification')
    db.session.commit()

    # Record video call result on blockchain
    event_type = 'VIDEO_VERIFICATION_PASSED' if passed else 'VIDEO_VERIFICATION_FAILED'
    block_hash, block_index = _record_on_chain(
        event_type, verif.user_id,
        {'pan_number': verif.pan_number, 'admin_remarks': verif.admin_remarks},
        recorded_by='admin'
    )
    if block_hash:
        verif.block_hash  = block_hash
    ...
    return jsonify({'id': verif_id, 'status': verif.status,
                    'video_status': verif.video_status,
                    'block_hash': block_hash, 'block_index': block_index})


# ── Helper: convert BeneficiaryVerification to dict ───────────────────────────
def _verif_to_dict(v):
    ...
    return {
        ...
        'aadhaar_doc_path': v.aadhaar_doc_path,
        'pan_doc_path':     v.pan_doc_path,
        'selfie_path':      v.selfie_path,
        'death_cert_path':  v.death_cert_path,
...
def _compute_match_score(selfie_path):
    """
    Placeholder face-match scorer.
    In production, replace with a real face-recognition library.
    Returns a score 0-100 based on simple image properties.
    """
    import random as _r
...
def blockchain_proxy(subpath):
    """Proxy /blockchain/* to the blockchain microservice on port 3001.
    Lets the React frontend use a single base URL (port 5000) for everything."""
    try:
        ...
        return jsonify({'error': 'Blockchain service unavailable',
                        'online': False,
...
if __name__ == '__main__':
    with app.app_context():
        ...
        # Migrate new columns (safe — no-op if they already exist)
        for col, col_type in [('block_hash', 'TEXT'), ('block_index', 'INT')]:
            try:
                db.session.execute(db.text(
                    f'ALTER TABLE beneficiary_verifications ADD COLUMN {col} {col_type}'
                ))
                db.session.commit()
